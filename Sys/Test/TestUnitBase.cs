	...
	public abstract class TestUnitBase
	{
		...
		protected void IterateResourceTestCases( string kind, string ext, bool expectFailures )
		{
			...
			while ( testResStream != null )
			{
				...
				logger.Debug( "" );
				logger.Debug( "==================================================================" );
				logger.Info( "Test case (valid): " + testCaseName );
			...
			if ( expectFailures )
			{
				...
				while ( testResStream != null )
				{
					...
					try
					{
						logger.Debug( "" );
						logger.Debug( "==================================================================" );
						logger.Info( "Test case (invalid): " + testCaseName );
						DoTest( kind, testResStream, testCaseName );
		...
		// ----------------------------------------------------------------------
		// members
		private static readonly ILogger logger = Logger.GetLogger( typeof( TestUnitBase ) );
